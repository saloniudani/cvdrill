...
public class SolrDataType extends RecordDataType {
  ...
  private final SolrSchemaResp cvSchema;

  public SolrDataType(SolrSchemaResp cvSchema) {
    ...
    for (SolrSchemaField cvSchemaField : cvSchema.getSchemaFields()) {
      ...
      if (solrFieldType.equals("string") || solrFieldType.equals("commaDelimited")
      ...
      } else if (solrFieldType.equals("double") || solrFieldType.equals("pdouble")
          || solrFieldType.equals("tdouble") || solrFieldType.equals("tlong")
          || solrFieldType.equals("rounded1024") || solrFieldType.equals("long")) {
        types.add(SqlTypeName.DOUBLE);
      } else if (solrFieldType.equals("date") || solrFieldType.equals("tdate")
          || solrFieldType.equals("timestamp") || solrFieldType.equals("epoch")) {
        ...
        SolrDataType.logger.trace(String.format(
            "PojoDataType doesn't yet support conversions from "
                + "type [%s] for field [%s].Defaulting to varchar.", solrFieldType,
