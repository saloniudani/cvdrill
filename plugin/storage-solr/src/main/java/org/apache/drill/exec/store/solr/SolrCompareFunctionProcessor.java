...
public class SolrCompareFunctionProcessor implements
  ...
  static {
    ...
    VALUE_EXPRESSION_CLASSES = builder.add(BooleanExpression.class).add(DateExpression.class)
        .add(DoubleExpression.class).add(FloatExpression.class).add(IntExpression.class)
        .add(LongExpression.class).add(QuotedString.class).add(TimeExpression.class).build();
  }
  ...
  static {
    ...
    COMPARE_FUNCTIONS_TRANSPOSE_MAP = builder.put("isnotnull", "isnotnull")
        .put("isNotNull", "isNotNull").put("is not null", "is not null").put("isnull", "isnull")
        .put("isNull", "isNull").put("is null", "is null").put("equal", "equal")
        .put("not_equal", "not_equal").put("greater_than_or_equal_to", "less_than_or_equal_to")
        .put("greater_than", "less_than").put("less_than_or_equal_to", "greater_than_or_equal_to")
        .put("less_than", "greater_than").build();
  }

  public static boolean isCompareFunction(String functionName) {
    return SolrCompareFunctionProcessor.COMPARE_FUNCTIONS_TRANSPOSE_MAP.keySet().contains(
        functionName);
  }
  ...
  public static SolrCompareFunctionProcessor process(FunctionCall call) {
    ...
    SolrCompareFunctionProcessor evaluator = new SolrCompareFunctionProcessor(functionName);

    if (valueArg != null) { // binary function

      if (SolrCompareFunctionProcessor.VALUE_EXPRESSION_CLASSES.contains(valueArg.getClass())) {
        SolrCompareFunctionProcessor.logger.info("functionName :: " + functionName);

        LogicalExpression swapArg = valueArg;
        valueArg = nameArg;
        nameArg = swapArg;
        evaluator.functionName = SolrCompareFunctionProcessor.COMPARE_FUNCTIONS_TRANSPOSE_MAP
            .get(functionName);
      }
  ...
  public SolrCompareFunctionProcessor(String functionName) {
    ...
    this.isEqualityFn = SolrCompareFunctionProcessor.COMPARE_FUNCTIONS_TRANSPOSE_MAP.containsKey(functionName) &&
        SolrCompareFunctionProcessor.COMPARE_FUNCTIONS_TRANSPOSE_MAP.get(functionName)
        .equals(functionName);
  }
  ...
  @Override
  public Boolean visitDecimal18Constant(Decimal18Expression decExpr, LogicalExpression value)
      throws RuntimeException {
    return false;
  }

  @Override
  public Boolean visitDecimal28Constant(Decimal28Expression decExpr, LogicalExpression value)
      throws RuntimeException {
    return false;
  }

  @Override
  public Boolean visitDecimal38Constant(Decimal38Expression decExpr, LogicalExpression value)
      throws RuntimeException {
    return false;
  }

  @Override
  public Boolean visitDecimal9Constant(Decimal9Expression decExpr, LogicalExpression value)
      throws RuntimeException {
    return false;
  }
