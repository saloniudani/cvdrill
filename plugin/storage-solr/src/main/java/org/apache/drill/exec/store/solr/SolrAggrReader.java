...
public class SolrAggrReader extends AbstractRecordReader {
    ...
    public SolrAggrReader(FragmentContext context, SolrSubScan config) {
        ...
        if (oCVSchema.getSchemaFields() != null) {
            schemaFieldMap = new HashMap<String, SolrSchemaField>(oCVSchema.getSchemaFields()
                                                                           .size());
        ...
        if (!solrAggrParams.isEmpty()) {
            QueryResponse queryRsp = solrClientApiExec.getSolrFieldStats(solrServerUrl,
                    solrCoreName, oCVSchema.getUniqueKey(), this.fields, sb);
    ...
    protected Collection<SchemaPath> transformColumns(
        ...
        if (!isStarQuery()) {
            logger.debug(
                " This is not a star query, restricting response to projected columns only " +
                projectedColumns);
        ...
        if (fieldStatsInfoMap != null) {
            for (SolrAggrParam solrAggrParam : solrAggrParams) {
                if (fieldStatsInfoMap.containsKey(solrAggrParam.getFieldName())) {
                    ...
                    MaterializedField m_field = MaterializedField.create(solrAggrParam.getFieldName(),
                            t.build());

                    try {
                        String key = solrAggrParam.getFunctionName() + "_" +
                            counter;
                        vectors.put(key,
            ...
            for (SolrAggrParam solrAggrParam : solrAggrParams) {
                ...
                if (vv.getClass().equals(NullableBigIntVector.class)) {
                    ...
                    if (functionName.equalsIgnoreCase("sum")) {
                        ...
                        logger.debug("yet to implement function type [ " +
                            functionName + " ]");
                    }
