...
ExprVisitor<Boolean, LogicalExpression, RuntimeException> {
  static final Logger logger = LoggerFactory.getLogger(SolrAggrFunctionProcessor.class);
  ...
  static {
    ...
    AGGR_FUNCTIONS_TRANSPOSE_MAP = builder.put("sum", "sum").put("count", "count")
        .put("min", "min").put("max", "max").put("avg", "avg").put("$sum0", "sum").build();
  }
  ...
  public static SolrAggrFunctionProcessor process(FunctionCall call) {
    ...
    SolrAggrFunctionProcessor evaluator = new SolrAggrFunctionProcessor(functionName);
    evaluator.functionName = SolrAggrFunctionProcessor.AGGR_FUNCTIONS_TRANSPOSE_MAP
        .get(functionName);
    if (evaluator.functionName != null) {
      if (valueArg != null) { // binary function

        if (SolrAggrFunctionProcessor.VALUE_EXPRESSION_CLASSES.contains(nameArg.getClass())) {
  ...
  public SolrAggrFunctionProcessor(String functionName) {
    ...
    this.isEqualityFn = SolrAggrFunctionProcessor.AGGR_FUNCTIONS_TRANSPOSE_MAP.containsKey(functionName) &&
        SolrAggrFunctionProcessor.AGGR_FUNCTIONS_TRANSPOSE_MAP.get(functionName)
        .equals(functionName);
  }
  ...
  public Boolean visitConvertExpression(ConvertExpression e,
      LogicalExpression value)
          throws RuntimeException {
    SolrAggrFunctionProcessor.logger.debug("SolrAggrFunctionProcessor :: visitConvertExpression ");
