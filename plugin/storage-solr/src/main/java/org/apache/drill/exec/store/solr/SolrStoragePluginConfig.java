...
public class SolrStoragePluginConfig extends StoragePluginConfig {
  ...
  static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(SolrStoragePluginConfig.class);
  @JsonProperty
  ...
  public SolrStoragePluginConfig(
      ...
      String solrServer, @JsonProperty("solrMode")
      String solrMode,
      @JsonProperty("solrCoreViewWorkspace")
      String solrCoreViewWorkspace,
      @JsonProperty("solrStorageProperties")
      SolrStorageProperties solrStorageProperties) {
    SolrStoragePluginConfig.logger.debug(
        "Initializing SOLR StoragePlugin configuration with solr server :: " +
            solrServer);
